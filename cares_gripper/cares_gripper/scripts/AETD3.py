...
from networks import AE_Actor as Actor
from networks import AE_Critic as Critic
...
class AETD3:
    def __init__(self, latent_size, action_num, device):
        ...
        self.k = 3 * 3  # number of stack frames, K*3  if I am using color images
        ...
        # Important
        # Latent_size+1 because I will inject the goal angle to the latent representation
        self.actor  = Actor(self.latent_size+1, self.action_num, self.encoder).to(self.device)
        self.critic = Critic(self.latent_size+1, self.action_num, self.encoder).to(self.device)

        self.actor_target  = Actor(self.latent_size+1, self.action_num, self.encoder).to(self.device)
        self.critic_target = Critic(self.latent_size+1, self.action_num, self.encoder).to(self.device)

        self.critic_target.load_state_dict(self.critic.state_dict())
        self.actor_target.load_state_dict(self.actor.state_dict())

        # here I do not include the goal angle in the z latent vector
        ...
        lr_encoder = 1e-4 # 1e-3
        lr_decoder = 1e-4 # 1e-3
    ...
    def select_action_from_policy(self, state, goal, evaluation=False, noise_scale=0.1):
        self.actor.eval()
        with torch.no_grad():
            state_tensor = torch.FloatTensor(state).to(self.device)
            state_tensor = state_tensor.unsqueeze(0)
            goal_tensor  = torch.Tensor([goal]).to(self.device) # goal needs to be added
            goal_tensor  = goal_tensor.unsqueeze(0)
            action       = self.actor(state_tensor, goal_tensor)
    ...
    def train_policy(self, experiences):
        ...
        goals       = torch.FloatTensor(np.asarray(goals)).to(self.device)  # Adding goal here

        # Reshape to batch_size
        rewards = rewards.unsqueeze(0).reshape(batch_size, 1)
        dones   = dones.unsqueeze(0).reshape(batch_size, 1)
        goals   = goals.unsqueeze(0).reshape(batch_size, 1)
        ...
        with torch.no_grad():
            ...
            target_q_values_one, target_q_values_two = self.critic_target(next_states, next_actions, goals)
            target_q_values = torch.minimum(target_q_values_one, target_q_values_two)

            q_target = rewards + self.gamma * (1 - dones) * target_q_values

        q_values_one, q_values_two = self.critic(states, actions, goals)
        ...
        if self.learn_counter % self.policy_update_freq == 0:
            actor_q_one, actor_q_two = self.critic(states, self.actor(states, goals, detach_encoder=True), goals, detach_encoder=True)
    ...
    def save_models(self, filename):
        ...
        torch.save(self.actor.state_dict(), f'models/{filename}_actor_ae.pht')
        torch.save(self.critic.state_dict(), f'models/{filename}_critic_ae.pht')
        ...
        torch.save(self.epm.state_dict(), f'models/{filename}_ensemble.pht')
