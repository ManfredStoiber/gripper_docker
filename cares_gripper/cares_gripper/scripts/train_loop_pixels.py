# this training loop just test without an autoencoder
...
def train(environment, agent, memory, learning_config, file_name):
    ...
    for total_step_counter in range(1, int(learning_config.max_steps_training) + 1):
        ...
        memory.add(state=state, action=action, reward=reward_extrinsic, next_state=next_state, done=done, goal=goal)
        state = next_state
...
def main():
    ...
    file_name     = f"{date_time_str}_RobotId{gripper_config.gripper_id}_EnvType{env_config.env_type}_ObsType{env_config.observation_type}_Seed{learning_config.seed}_Pixel_TD3"
