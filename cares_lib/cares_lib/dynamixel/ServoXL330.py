...
class ServoXL330(object):
    ...
    def enable(self, control_mode = ControlMode.JOINT):
        try:
            self.disable_torque()
            self.set_control_mode(control_mode.value)  # default set to joint mode to avoid moving on start up
            self.limit_torque()
            self.limit_speed()
            self.enable_torque()
            self.turn_on_LED()
    ...
    def step(self):
        ...
        try:
            ...
            current_velocity = ServoXL330.velocity_to_int(current_state["velocity"])
    ...
    def control_mode(self):
        try:
            current_mode, dxl_comm_result, dxl_error = self.packet_handler.read1ByteTxRx(self.port_handler,
                                                                                         self.motor_id, ServoXL330.addresses[
                                                                                             "control_mode"])
            self.process_result(dxl_comm_result, dxl_error,
    ...
    def set_control_mode(self, new_mode):
        try:
            ...
            dxl_comm_result, dxl_error = self.packet_handler.write1ByteTxRx(self.port_handler, self.motor_id,
                                                                            ServoXL330.addresses["control_mode"], new_mode)
    ...
    def move_velocity(self, target_velocity):
        ...
        try:
            ...
            processed_velocity = ServoXL330.velocity_to_bytes(target_velocity)
            dxl_comm_result, dxl_error = self.packet_handler.write2ByteTxRx(self.port_handler, self.motor_id,
                                                                            ServoXL330.addresses["moving_speed"],
                                                                            processed_velocity)
            self.process_result(dxl_comm_result, dxl_error,
    ...
    def current_load(self):
        try:
            data_read, dxl_comm_result, dxl_error = self.packet_handler.read2ByteTxRx(self.port_handler, self.motor_id,
                                                                                      ServoXL330.addresses["current_load"])
    ...
    def turn_on_LED(self):
        try:
            dxl_comm_result, dxl_error = self.packet_handler.write1ByteTxRx(self.port_handler, self.motor_id,
                                                                            ServoXL330.addresses["led"], self.LED_colour)
            self.process_result(dxl_comm_result, dxl_error,
    ...
    def limit_torque(self):
        try:
            # TODO:
            ## dxl_comm_result, dxl_error = self.packet_handler.write2ByteTxRx(self.port_handler, self.motor_id,
            ##                                                                 ServoXL330.addresses["torque_limit"],
            ##                                                                 self.torque_limit)
            return # TODO delete this line
            self.process_result(dxl_comm_result, dxl_error,
    ...
    def enable_torque(self):
        try:
            dxl_comm_result, dxl_error = self.packet_handler.write2ByteTxRx(self.port_handler, self.motor_id,
                                                                            ServoXL330.addresses["torque_enable"], 1)
            self.process_result(dxl_comm_result, dxl_error,
    ...
    def disable_torque(self):
        try:
            dxl_comm_result, dxl_error = self.packet_handler.write1ByteTxRx(self.port_handler, self.motor_id,
                                                                            ServoXL330.addresses["torque_enable"], 0)
            self.process_result(dxl_comm_result, dxl_error,
